...
\AddToShipoutPictureBG{%
    ...
    \AtPageLowerLeft{%
        \rlap{\includegraphics[width=\paperwidth]{design/Footer.png}}%
        \hspace{190.5mm}%
        \raisebox{9mm}{%
            \makebox[13.5mm][c]{%
...
Kemampuan tersebut membuat fuzzy logic sesuai digunakan pada sistem yang kondisi lingkungannya tidak selalu memiliki batas yang tegas, seperti sistem pemantauan tanaman yang dibahas pada modul ini. Untuk menentukan derajat keanggotaan, digunakan \textit{membership function}. \textit{Membership function} berfungsi untuk menentukan seberapa besar suatu nilai sensor termasuk ke dalam kategori tertentu. Bentuk \textit{membership function} yang umum digunakan antara lain berbentuk segitiga (\textit{triangular}) dan trapesium (\textit{trapezoidal}).
...
Pada percobaan ini digunakan \textit{membership function} berbentuk segitiga. Untuk fungsi segitiga, derajat keanggotaan dapat dihitung menggunakan persamaan berikut.
...
Selain dibedakan berdasarkan jenis input yang digunakan, sistem fuzzy logic juga dapat dibedakan berdasarkan metode inferensi yang digunakan pada tahap penalaran dan defuzzifikasi. Beberapa metode inferensi yang umum digunakan antara lain metode Mamdani, metode Sugeno (\textit{Takagi-Sugeno-Kang}), dan metode Tsukamoto.

Sistem pada modul ini menggunakan keluaran \textit{rule} berupa nilai \textit{crisp} tunggal (\textit{singleton}) untuk setiap kategori, yaitu 0, 50, dan 100, yang kemudian digabungkan langsung menggunakan rata-rata berbobot (\textit{weighted average}) tanpa melalui proses agregasi himpunan fuzzy maupun perhitungan \textit{centroid}. Karakteristik tersebut sesuai dengan metode Sugeno orde-nol (\textit{zero-order Sugeno}), sehingga proses penalaran dan defuzzifikasi pada modul ini mengikuti metode Sugeno.
...
Tahap input merupakan tahap awal dalam proses pengolahan fuzzy. Pada tahap ini, sistem membaca kondisi lingkungan menggunakan sensor DHT22 untuk mengukur suhu dan \textit{Capacitive Soil Moisture Sensor} untuk mengukur kelembapan tanah. Hasil pembacaan sensor berupa nilai \textit{crisp}, yaitu nilai dalam bentuk angka yang dapat diukur secara langsung, misalnya suhu sebesar $28^\circ$C dan kelembapan tanah sebesar $32\%$.
...
Fuzzifikasi merupakan tahap mengubah nilai input \textit{crisp} menjadi derajat keanggotaan fuzzy pada masing-masing himpunan yang telah ditentukan. Pada tahap ini, nilai suhu dan kelembapan tanah yang diperoleh dari sensor dimasukkan ke dalam \textit{membership function} masing-masing variabel untuk mengetahui seberapa besar nilai tersebut termasuk ke dalam kategori tertentu. Bentuk \textit{membership function} untuk masing-masing variabel input ditampilkan pada gambar berikut.
...
Sebagai contoh, berdasarkan pengujian pada sistem, sensor membaca suhu sebesar $27^\circ$C. Berdasarkan \textit{membership function}, suhu tersebut termasuk dalam kategori dingin dan sedang. Perhitungan derajat keanggotaannya adalah sebagai berikut.

Untuk kategori dingin dengan parameter $[0,24,28]$:
...
Untuk kategori sedang dengan parameter $[25,30,45]$:
...
Penalaran merupakan tahap menentukan hasil berdasarkan hubungan antara input yang telah difuzzifikasi dengan aturan dasar (\textit{rule}) yang telah disusun sebelumnya. Sistem ini menggunakan rule yang menggabungkan kategori suhu dengan kategori kelembapan tanah menggunakan operator AND. Dalam fuzzy logic, operator AND diselesaikan dengan mengambil nilai minimum dari kedua derajat keanggotaan yang digabungkan.
...
Berdasarkan derajat keanggotaan yang telah diperoleh, yaitu suhu dingin sebesar $0{,}25$, suhu sedang sebesar $0{,}4$, dan kelembapan tanah lembap sebesar $1{,}0$, maka nilai $\alpha$ untuk setiap rule dihitung sebagai berikut.
...
Rule 2 menggunakan kondisi suhu dingin dan kelembapan lembap sehingga menghasilkan kategori kuning. Rule 5 menggunakan kondisi suhu sedang dan kelembapan lembap sehingga menghasilkan kategori hijau. Kedua rule tersebut dapat dihitung karena seluruh derajat keanggotaan yang terlibat bernilai lebih besar dari nol.

Rule yang aktif, yaitu rule dengan $\alpha > 0$, adalah Rule 2 dan Rule 5. Karena masing-masing rule menghasilkan kategori keluaran yang berbeda, yaitu Rule 2 menghasilkan kategori kuning dan Rule 5 menghasilkan kategori hijau, tidak diperlukan proses agregasi dengan nilai maksimum sebagaimana dilakukan apabila terdapat beberapa rule dengan kategori keluaran yang sama. Nilai $\alpha$ masing-masing rule langsung menjadi kekuatan akhir kategori keluarannya.

\[
\alpha_{\text{kuning}} = \alpha_2 = 0{,}25
\]

\[
\alpha_{\text{hijau}} = \alpha_5 = 0{,}4
\]
...
Metode yang digunakan adalah rata-rata berbobot (\textit{weighted average}), sesuai dengan metode Sugeno orde-nol, yang menghitung nilai rata-rata dari seluruh rule yang aktif dengan bobot berupa kekuatan masing-masing rule ($\alpha$). Nilai yang dibobotkan berupa nilai \textit{crisp} tunggal (\textit{singleton}) dari kategori keluaran rule tersebut, yaitu 0 untuk merah, 50 untuk kuning, dan 100 untuk hijau. Persamaan yang digunakan adalah sebagai berikut.
...
Melanjutkan contoh sebelumnya, Rule 5 menghasilkan keluaran hijau dengan kekuatan $0{,}4$, sedangkan Rule 2 menghasilkan keluaran kuning dengan kekuatan $0{,}25$. Maka nilai keluaran akhir dihitung sebagai berikut.

\begin{align}
z &=
\frac{(0{,}4 \times 100)+(0{,}25 \times 50)}
{0{,}4+0{,}25}
\\[4pt]
&=
\frac{40+12{,}5}{0{,}65}
...
Nilai $\approx 80{,}77$ merupakan hasil defuzzifikasi atau nilai keluaran akhir sistem. Nilai tersebut kemudian digunakan pada tahap output untuk menentukan warna LED yang akan menyala. Dengan demikian, hasil dari beberapa rule aktif dapat digabungkan menjadi satu nilai keluaran yang dapat digunakan oleh sistem.

\textbf{e) Output}

Nilai \textit{crisp} ($z$) yang diperoleh dari tahap defuzzifikasi selanjutnya dipetakan ke dalam kategori LED berdasarkan rentang nilai yang telah ditentukan. Pemetaan tersebut digunakan untuk mengubah nilai keluaran menjadi indikator kondisi yang dapat diamati secara langsung oleh pengguna. Rentang kategori LED yang digunakan pada sistem adalah sebagai berikut.
