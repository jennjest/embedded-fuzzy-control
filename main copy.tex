...
Sebagai contoh, sensor membaca suhu sebesar $27^\circ$C. Berdasarkan membership function, suhu tersebut termasuk dalam kategori sedang dan panas. Perhitungan derajat keanggotaannya adalah sebagai berikut.
...
Berikut adalah daftar istilah penting yang digunakan dalam modul ini beserta penjelasan sederhananya.

\begin{itemize}[label={}, leftmargin=0pt, itemsep=0.35cm]
    \item \textbf{Aktuator}\\
    Komponen yang mengubah keputusan sistem menjadi aksi atau tampilan yang dapat diamati secara langsung, misalnya LED yang menyala.

    \item \textbf{Analog to Digital Converter (ADC)}\\
    Bagian pada mikrokontroler yang mengubah sinyal analog dari sensor menjadi angka digital agar dapat diolah oleh program.

    \item \textbf{Capacitive Soil Moisture Sensor}\\
    Sensor yang digunakan untuk mengukur tingkat kelembapan tanah atau media tanam.

    \item \textbf{Crisp Logic (Logika Tegas)}\\
    Cara berpikir yang hanya mengenal dua kemungkinan, yaitu benar atau salah, sehingga suatu nilai hanya dapat masuk ke satu kategori.

    \item \textbf{Defuzzifikasi}\\
    Tahap mengubah hasil penalaran fuzzy menjadi satu nilai akhir (nilai crisp) yang dapat digunakan sistem untuk mengambil keputusan.

    \item \textbf{Derajat Keanggotaan}\\
    Angka antara 0 sampai 1 yang menunjukkan seberapa besar suatu nilai termasuk ke dalam suatu kategori (himpunan fuzzy).

    \item \textbf{DHT22}\\
    Sensor yang digunakan untuk mengukur suhu dan kelembapan udara di sekitar tanaman.

    \item \textbf{Embedded System}\\
    Sistem komputer kecil yang dipasang di dalam suatu alat untuk menjalankan satu fungsi tertentu, seperti membaca sensor dan menyalakan LED.

    \item \textbf{ESP32}\\
    Jenis mikrokontroler yang digunakan sebagai pengendali utama pada percobaan dalam modul ini.

    \item \textbf{Fungsi Keanggotaan (Membership Function)}\\
    Rumus atau grafik yang digunakan untuk menghitung derajat keanggotaan suatu nilai sensor terhadap suatu kategori tertentu.

    \item \textbf{Fuzzifikasi}\\
    Tahap mengubah nilai sensor menjadi derajat keanggotaan pada beberapa kategori fuzzy.

    \item \textbf{Himpunan Fuzzy}\\
    Kategori nilai dalam logika fuzzy yang memiliki batas tidak tegas, misalnya kategori suhu ``sedang''.

    \item \textbf{Logika Fuzzy (Fuzzy Logic)}\\
    Metode pengambilan keputusan yang memungkinkan suatu nilai memiliki tingkat keanggotaan pada lebih dari satu kategori sekaligus, tidak hanya benar atau salah.

    \item \textbf{Metode Sugeno}\\
    Salah satu metode pengolahan fuzzy yang menghasilkan nilai akhir dengan menghitung rata-rata berbobot dari nilai output setiap rule yang aktif.

    \item \textbf{Mikrokontroler}\\
    Chip komputer kecil yang mengatur seluruh proses dalam embedded system, mulai dari membaca sensor hingga mengendalikan keluaran.

    \item \textbf{Nilai Crisp}\\
    Nilai akhir berupa satu angka tunggal, bukan lagi derajat keanggotaan, yang dihasilkan setelah proses defuzzifikasi.

    \item \textbf{Penalaran (Inferensi)}\\
    Tahap menentukan hasil berdasarkan rule (aturan) yang sesuai dengan kondisi input yang sedang terjadi.

    \item \textbf{Rata-rata Berbobot (Weighted Average)}\\
    Cara menghitung nilai akhir dengan mengalikan setiap nilai output rule dengan kekuatannya, lalu membagi totalnya dengan jumlah seluruh kekuatan rule yang aktif.

    \item \textbf{Rule (Aturan Fuzzy)}\\
    Pernyataan ``jika ... maka ...'' yang menghubungkan kondisi input, misalnya suhu dan kelembapan tanah, dengan kategori output sistem.

    \item \textbf{Sensor}\\
    Komponen yang mendeteksi kondisi fisik di sekitar, seperti suhu atau kelembapan, dan mengubahnya menjadi data yang dapat dibaca oleh mikrokontroler.

    \item \textbf{Singleton}\\
    Nilai tunggal yang mewakili suatu kategori output pada metode Sugeno, misalnya 0 untuk kondisi buruk, 50 untuk sedang, dan 100 untuk baik.
\end{itemize}
...
Sinha, M. and Tiwary, R. (2026). Development of an ESP32-Based Fuzzy Logic Smart Farming Monitoring System for Enhanced Agricultural Productivity. International Journal For Multidisciplinary Research. doi: 10.36948/ijfmr.2026.v08i01.69732.

\vspace{0.3cm}

\noindent
\hangindent=1.2cm\hangafter=1
Wijayakusuma, P., Hakim, G. P. N. and Li, B. (2025). TerraGrow: Integrated platform for real time plant monitoring and automated watering system with IoT and fuzzy Sugeno Algorithm. HardwareX, 24. doi: 10.1016/j.ohx.2025.e00724.
